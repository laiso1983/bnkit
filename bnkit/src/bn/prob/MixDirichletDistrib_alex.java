 ...
 * 
*/
public class MixDirichletDistrib_alex extends MixtureDistrib implements Serializable {
     ...
     * @param weight1
     */
    public MixDirichletDistrib_alex(DirichletDistrib d1, double weight1) {
        super(d1, weight1);
     ...
     * @param ComponentNum
     */
    public MixDirichletDistrib_alex(Enumerable domain, int ComponentNum, int[][] data) {
        super();
...
//            super.addDistrib(new DirichletDistrib(new EnumDistrib(domain, data[rand.nextInt(data.length)]), rand.nextInt(90) + 10), rand.nextDouble());
     ...
     * @param ComponentNum
     */
    public MixDirichletDistrib_alex(Enumerable domain, int ComponentNum) {
        super();
//        Random rand = new Random(System.currentTimeMillis());
...
//                        bw.write(d.get(j) + "\t");
     ...
     *
     * @param args
     * filename - segmented ChIP-peak file
     * minBins - minimum number of bins/clusters to test
     * maxBins - maximum number of bins/clusters to test
     * save - optional argument; if true - saves all generated results
     *          if false - saves only the best cluster size result
     */
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 3) {
            System.out.println("Usage: filename, minBins, maxBins");
            System.out.println("Usage: filename, minBins, maxBins, save?");
            System.exit(1);
            ...
            if (args[3].equals("true")) {
                ...
                System.out.println("Save must be either 'true' or 'false'");
                System.exit(1);
            }
        } else {
            System.out.println("Usage: filename, minBins, maxBins");
        ...
        Map<Integer, MixDirichletDistrib> mddOutputs = new HashMap<>();
        double minComp = 0.0;
        int minBin = 0;
        if (save) { //Record all resulting dirichlet distributions
            for ( int nbins = min; nbins < max; nbins++) {
                ...
                mddOutputs.put(nbins, dis);
                dis.saveAlphas(filename);
                double comp = dis.getDLBest() +  dis.getComplexity(data);
                fComp[nbins - min] = comp;
	            dis.saveClusters(data, filename);
            }
        } else { //Only record the optimal cluster size result
            for ( int nbins = min; nbins < max; nbins++) {
                System.out.println("nbins = " +nbins);
                MixDirichletDistrib dis = new MixDirichletDistrib(domain, nbins, data);
                dis.learnParametersFlip(data); //For ChIP-seq peak data that has unknown orientation
                ...
                double comp = dis.getDLBest() +  dis.getComplexity(data);
                fComp[nbins - min] = comp;
                if (nbins == min) {// first result so it automatically becomes the min value
                    minComp = comp;
                    minBin = nbins;
                }
                if (comp < minComp) {
            ...
            MixDirichletDistrib minResult = mddOutputs.get(minBin);
            minResult.saveAlphas(filename);
            minResult.saveClusters(data, filename);
        }
